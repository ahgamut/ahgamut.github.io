...
`loslib.c` reuiqres the `setlocale()` and `LC_*` from `locale.h`, which is
...
![]({% link assets/images/linux_screen.png %})

Awesome. How about Windows?

![]({% link assets/images/windows_screen.png %})
...
is currently at release 0.0.2, so there is a lot of time.

Hopefully this means that other languages that have source code completely in C
can also be compiled once and run anywhere. Actually Portable Python next,
maybe?
